...
const Shows_Trending = () => {

  const { loading, trendingShows, getTrendingShows } = useMoviestore()
  const trendingShowsArr = trendingShows.slice(0, 10)
  ...
  const slideRight = () => {
    ...
    const maxPage = Math.ceil(trendingShows.length / cardsPerPage) - 1
    ...
    <div className='px-6 md:px-12 lg:px-20'>
      <div className="popular-card">
        ...
        >
          ...
          {trendingShowsArr.map((popularMovie) => (
            <div
            onClick={()=>{navigate(`/recommendations/naviage/${popularMovie.id}`)}}
              key={popularMovie.id}
            ...
            >
              ...
              <div className='flex justify-between w-full px-5 mb-5'>
                <div className='flex items-start gap-1 bg-[#070707] text-white py-1 px-3 rounded-full'>
                  ...
                  <p className='text-[14px]'>{popularMovie.first_air_date.split("-")[0]}</p>
                </div>
